...
> **Note:** Claude ticker review is enabled. After generating the workbook, the engine exports a consolidated CSV (Investment Master + Speculative Investments) and POSTs it to the Claude API. Claude returns an acknowledgment and up to 5 ranked ticker recommendations, saved as `ticker_review_MMDDYYYY.json` in the sector's `ticker-review/` folder. Requires the `ANTHROPIC_API_KEY` GitHub secret.
...
| `ANTHROPIC_API_KEY` | Anthropic API key for Claude ticker review |
...
4. Exports a consolidated CSV and POSTs it to Claude for ticker review
5. Saves Claude's acknowledgment and top-5 recommendations to `ticker-review/ticker_review_MMDDYYYY.json`
6. Commits and pushes any changed files
